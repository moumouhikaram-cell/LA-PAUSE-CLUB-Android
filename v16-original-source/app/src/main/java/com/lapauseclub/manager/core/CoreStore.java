...
public final class CoreStore extends SQLiteOpenHelper {
    ...
    @Override public void onCreate(SQLiteDatabase db){
        ...
        putMeta(db,"master_contract_version","2026-09-04-v11-command-scope");
    }
    ...
    @Override public void onUpgrade(SQLiteDatabase db,int oldVersion,int newVersion){
        ...
        try{
            ...
            if(oldVersion<11){CoreCommandSchemaV11.create(db);CoreCommandSchemaV11.migrateLegacy(db,System.currentTimeMillis());}
            putMeta(db,"core_schema_version",String.valueOf(DB_VERSION));
            putMeta(db,"master_contract_version","2026-09-04-v11-command-scope");
            putMeta(db,"migration_mode","DOMAIN_DUAL_WRITE");
    ...
    private static void createAllSchemas(SQLiteDatabase db){
        CoreDomainSchemaV2.create(db);CoreOperationalSchemaP1.create(db);CoreBusinessSchemaP1.create(db);CoreDeviceSchemaP2.create(db);CoreOwnerSchemaP3.create(db);CorePlayerSchemaP4.create(db);CoreSaasSchemaP5.create(db);CoreSyncSchemaV8.create(db);CoreCommandSchemaV9.create(db);CoreTenantSchemaV10.create(db);CoreCommandSchemaV11.create(db);
    }
    ...
    public synchronized String commitCommand(String commandJson,String nextStateJson,String eventJson){
        ...
        try{
            ...
            String idem=required(cmd,"idempotencyKey"),tenant=required(cmd,"tenantId"),venue=required(cmd,"venueId"),branch=required(cmd,"branchId"),actor=required(cmd,"actorId"),device=required(cmd,"originDeviceId");
            String scopeError=validateScope(next.optJSONObject("saas"),tenant,venue,branch,"state");if(scopeError!=null)return error("SCOPE_MISMATCH",scopeError);
            scopeError=validateScope(evt,tenant,venue,branch,"event");if(scopeError!=null)return error("SCOPE_MISMATCH",scopeError);
            ...
            String declaredModule=blankToNull(cmd.optString("moduleId")),declaredPermission=blankToNull(cmd.optString("requiredPermission"));
            ...
            String fingerprint=sha256(tenant+"|"+venue+"|"+branch+"|"+type+"|"+idem+"|"+(expected==null?"":expected)+"|"+(entityType==null?"":entityType)+"|"+(entityId==null?"":entityId)+"|"+(declaredModule==null?"":declaredModule)+"|"+(declaredPermission==null?"":declaredPermission)+"|"+payload);
            SQLiteDatabase db=getWritableDatabase();
            String replay=CoreCommandSchemaV11.commandResult(db,tenant,venue,branch,idem,fingerprint);if(replay!=null)return replay;
            db.beginTransaction();
            try{
                replay=CoreCommandSchemaV11.commandResult(db,tenant,venue,branch,idem,fingerprint);if(replay!=null){db.setTransactionSuccessful();return replay;}
                long currentRevision=CoreCommandSchemaV11.entityRevision(db,tenant,venue,branch,entityType,entityId);
                if(expected!=null&&expected!=currentRevision)return rejectInTx(db,commandId,tenant,venue,branch,type,idem,expected,entityType,entityId,actor,device,correlation,payload,fingerprint,"REVISION_CONFLICT","Expected "+expected+" but current is "+currentRevision,now);
                String previousJson=latestStateJson(db,tenant,venue,branch);JSONObject previous=previousJson.isEmpty()?new JSONObject():new JSONObject(previousJson);
                CoreAuthorityGuardV12.Decision authority=CoreAuthorityGuardV12.authorize(cmd,previous,now);
                if(!authority.allowed)return rejectInTx(db,commandId,tenant,venue,branch,type,idem,expected,entityType,entityId,actor,device,correlation,payload,fingerprint,authority.code,authority.message,now);
                ...
                if(transitionError!=null)return rejectInTx(db,commandId,tenant,venue,branch,type,idem,expected,entityType,entityId,actor,device,correlation,payload,fingerprint,"BUSINESS_RULE",transitionError,now);
                CoreCommandSchemaV11.insertPendingCommand(db,commandId,tenant,venue,branch,type,idem,expected,entityType,entityId,actor,device,correlation,payload,fingerprint,now);
                long newRevision=(entityType!=null&&entityId!=null)?currentRevision+1:currentRevision;
                if(entityType!=null&&entityId!=null){CoreCommandSchemaV11.setEntityRevision(db,tenant,venue,branch,entityType,entityId,newRevision,now);applyRevision(next,entityType,entityId,newRevision,now);}
                ...
                String eventType=nonBlank(evt.optString("eventType"),eventTypeForCommand(type));String eventId=nonBlank(evt.optString("eventId"),"evt-"+UUID.randomUUID());String eventIdem=nonBlank(evt.optString("idempotencyKey"),"event:"+commandId+":"+eventType);String station=blankToNull(evt.optString("stationId")),severity=nonBlank(evt.optString("severity"),"INFO"),causation=blankToNull(evt.optString("causationId",commandId));Object eventPayload=evt.has("payload")?evt.opt("payload"):payloadObj;String eventPayloadJson=eventPayload==null||eventPayload==JSONObject.NULL?"{}":String.valueOf(eventPayload);
                CoreCommandSchemaV11.appendEvent(db,eventId,eventType,tenant,venue,branch,station,device,entityType,entityId,actor,now,eventPayloadJson,correlation,causation,eventIdem,severity,Math.max(1,evt.optInt("schemaVersion",1)));
                String prevHash=CoreCommandSchemaV11.lastAuditHash(db,tenant,venue,branch);String auditId="audit-"+UUID.randomUUID();String auditCanonical=auditId+"|"+tenant+"|"+venue+"|"+branch+"|"+actor+"|"+device+"|"+type+"|"+(entityId==null?"":entityId)+"|"+now+"|"+prevHash+"|"+commandId;String auditHash=sha256(auditCanonical);CoreCommandSchemaV11.appendAudit(db,auditId,tenant,venue,branch,actor,device,entityType,entityId,type,payload,now,prevHash,auditHash,correlation,commandId);
                JSONObject result=new JSONObject();result.put("ok",true);result.put("commandId",commandId);result.put("idempotencyKey",idem);result.put("tenantId",tenant);result.put("venueId",venue);result.put("branchId",branch);result.put("moduleId",authority.moduleId);result.put("requiredPermission",authority.requiredPermission);result.put("authorityMode",authority.authorityMode);result.put("newRevision",newRevision);result.put("eventId",eventId);result.put("state",next);String resultJson=result.toString();CoreCommandSchemaV11.markApplied(db,commandId,resultJson,now);db.setTransactionSuccessful();return resultJson;
            }finally{db.endTransaction();}
        }catch(Exception e){return error("MALFORMED",e.getMessage()==null?"Command rejected":e.getMessage());}
    }

    private static String rejectInTx(SQLiteDatabase db,String commandId,String tenant,String venue,String branch,String type,String idem,Long expected,String entityType,String entityId,String actor,String device,String correlation,String payload,String fingerprint,String code,String message,long now){
        CoreCommandSchemaV11.insertPendingCommand(db,commandId,tenant,venue,branch,type,idem,expected,entityType,entityId,actor,device,correlation,payload,fingerprint,now);CoreCommandSchemaV11.markRejected(db,commandId,code,message,now);JSONObject out=new JSONObject();try{out.put("ok",false);out.put("code",code);out.put("message",message);out.put("commandId",commandId);out.put("tenantId",tenant);out.put("venueId",venue);out.put("branchId",branch);}catch(Exception ignored){}db.setTransactionSuccessful();return out.toString();
    }

    private static String validateScope(JSONObject scoped,String tenant,String venue,String branch,String label){
        if(scoped==null)return null;
        String t=scoped.optString("tenantId","").trim(),v=scoped.optString("venueId","").trim(),b=scoped.optString("branchId","").trim();
        if(!t.isEmpty()&&!tenant.equals(t))return label+" tenantId does not match command scope";
        if(!v.isEmpty()&&!venue.equals(v))return label+" venueId does not match command scope";
        if(!b.isEmpty()&&!branch.equals(b))return label+" branchId does not match command scope";
        return null;
    }
    ...
    private static String latestStateJson(SQLiteDatabase db,String tenant,String venue,String branch){try(Cursor c=db.rawQuery("SELECT state_json,checksum_sha256 FROM state_snapshots ORDER BY created_at_ms DESC LIMIT "+MAX_SNAPSHOTS,null)){while(c.moveToNext()){String j=c.getString(0),h=c.getString(1);try{JSONObject root=new JSONObject(j);if(h.equals(sha256(j))&&stateMatchesScope(root,tenant,venue,branch))return j;}catch(Exception ignored){}}}return "";}
    private static boolean stateMatchesScope(JSONObject root,String tenant,String venue,String branch){JSONObject s=root==null?null:root.optJSONObject("saas");if(s==null)return "local".equals(tenant)||"local-la-pause".equals(tenant);String t=s.optString("tenantId","").trim(),v=s.optString("venueId","").trim(),b=s.optString("branchId","").trim();return tenant.equals(t)&&venue.equals(v)&&branch.equals(b);}

    public synchronized JSONObject getTimelineJson(){JSONObject out=new JSONObject();JSONArray ss=new JSONArray(),ev=new JSONArray();SQLiteDatabase db=getReadableDatabase();String[] scope=activeScopeIds(db);try(Cursor c=db.rawQuery("SELECT id,source,data_revision,created_at_ms,checksum_sha256,state_json FROM state_snapshots ORDER BY created_at_ms DESC LIMIT "+MAX_SNAPSHOTS,null)){while(c.moveToNext()){String stateJson=c.getString(5);try{JSONObject root=new JSONObject(stateJson);if(!c.getString(4).equals(sha256(stateJson))||!stateMatchesScope(root,scope[0],scope[1],scope[2]))continue;JSONObject x=new JSONObject();x.put("id",c.getString(0));x.put("source",c.getString(1));x.put("revision",c.getLong(2));x.put("createdAtMs",c.getLong(3));x.put("checksum",c.getString(4));ss.put(x);}catch(Exception ignored){}}}catch(Exception ignored){}try(Cursor c=db.rawQuery("SELECT event_id,event_type,entity_id,server_timestamp_ms FROM domain_events_v11 WHERE tenant_id=? AND venue_id=? AND branch_id=? ORDER BY server_timestamp_ms DESC LIMIT 60",scope)){while(c.moveToNext()){JSONObject x=new JSONObject();x.put("eventId",c.getString(0));x.put("eventType",c.getString(1));x.put("entityId",c.isNull(2)?JSONObject.NULL:c.getString(2));x.put("eventAtMs",c.getLong(3));ev.put(x);}}catch(Exception ignored){}try{out.put("tenantId",scope[0]);out.put("venueId",scope[1]);out.put("branchId",scope[2]);out.put("snapshots",ss);out.put("events",ev);}catch(Exception ignored){}return out;}

    public synchronized JSONObject getStatusJson(){SQLiteDatabase db=getReadableDatabase();JSONObject out=new JSONObject();try{JSONObject a2=CoreDomainSchemaV2.status(db),tenant=CoreTenantSchemaV10.status(db),p1=CoreOperationalSchemaP1.status(db),biz=CoreBusinessSchemaP1.status(db),p2=CoreDeviceSchemaP2.status(db),p3=CoreOwnerSchemaP3.status(db),p4=CorePlayerSchemaP4.status(db),p5=CoreSaasSchemaP5.status(db),sync=CoreSyncSchemaV8.status(db);String tid=nonBlank(tenant.optString("tenantId"),"local"),vid=nonBlank(tenant.optString("venueId"),"local"),bid=nonBlank(tenant.optString("branchId"),"local");JSONObject cmd=new JSONObject(CoreCommandSchemaV11.statusJson(db,tid,vid,bid));JSONArray rr=tenant.optJSONArray("resourceRegistry");out.put("coreVersion","master-v2-command-v11");out.put("dbSchemaVersion",DB_VERSION);out.put("masterContractVersion",getMeta(db,"master_contract_version",""));out.put("migrationMode",getMeta(db,"migration_mode","DOMAIN_DUAL_WRITE"));out.put("operatingMode",getMeta(db,"operating_mode","STANDALONE"));out.put("authorityState",getMeta(db,"authority_state","TABLET_PRIMARY"));out.put("legacySchemaVersion",parseLong(getMeta(db,"legacy_schema_version","0")));out.put("legacyDataRevision",parseLong(getMeta(db,"legacy_data_revision","0")));out.put("lastMirrorAtMs",parseLong(getMeta(db,"last_mirror_at_ms","0")));out.put("snapshotCount",scopedSnapshotCount(db,tid,vid,bid));out.put("resourceCount",rr==null?0:rr.length());out.put("eventCount",cmd.optLong("canonicalEventCount",0));out.put("pendingSyncCount",cmd.optLong("canonicalOutboxPending",0));out.put("checkpointCount",scalar(db,"SELECT COUNT(*) FROM migration_checkpoints"));out.put("venueProfile",tenant.optJSONObject("venueProfile"));out.put("resourceRegistry",rr);out.put("tenantScopeV10",tenant);out.put("legacyDomainV2",a2);out.put("legacyCommandV9Preserved",true);out.put("domainAuthority",a2.optJSONArray("domainAuthority"));out.put("normalizedDomains",tenant.optJSONArray("normalizedDomains"));out.put("p1Operational",p1);out.put("p1Business",biz);out.put("p2Device",p2);out.put("p3Owner",p3);out.put("p4Player",p4);out.put("p5Saas",p5);out.put("webParitySync",sync);out.put("commandCore",cmd);out.put("authorityProgress","MASTER_V2_NATIVE_ENTITLEMENT_RBAC_V12");out.put("tenantIsolationStage","RESOURCE_COMMAND_AND_AUTHORITY_SCOPE");out.put("legacyStillAuthoritative",true);out.put("networkRequired",false);}catch(Exception ignored){}return out;}
    ...
    private static String[] activeScopeIds(SQLiteDatabase db){JSONObject t=CoreTenantSchemaV10.status(db);return new String[]{nonBlank(t.optString("tenantId"),"local"),nonBlank(t.optString("venueId"),"local"),nonBlank(t.optString("branchId"),"local")};}
    private static long scopedSnapshotCount(SQLiteDatabase db,String tenant,String venue,String branch){long n=0;try(Cursor c=db.rawQuery("SELECT state_json,checksum_sha256 FROM state_snapshots ORDER BY created_at_ms DESC LIMIT "+MAX_SNAPSHOTS,null)){while(c.moveToNext()){String j=c.getString(0);try{JSONObject root=new JSONObject(j);if(c.getString(1).equals(sha256(j))&&stateMatchesScope(root,tenant,venue,branch))n++;}catch(Exception ignored){}}}return n;}
