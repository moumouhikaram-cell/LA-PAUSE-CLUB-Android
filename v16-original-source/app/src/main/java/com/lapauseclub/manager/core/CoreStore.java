...
public final class CoreStore extends SQLiteOpenHelper {
    ...
    public synchronized String commitCommand(String commandJson,String nextStateJson,String eventJson){
        ...
        try{
            ...
            String fingerprint=sha256(tenant+"|"+venue+"|"+branch+"|"+type+"|"+idem+"|"+(expected==null?"":expected)+"|"+(entityType==null?"":entityType)+"|"+(entityId==null?"":entityId)+"|"+payload);
            ...
            try{
                ...
                JSONObject result=new JSONObject();result.put("ok",true);result.put("commandId",commandId);result.put("idempotencyKey",idem);result.put("tenantId",tenant);result.put("venueId",venue);result.put("branchId",branch);result.put("newRevision",newRevision);result.put("eventId",eventId);result.put("state",next);String resultJson=result.toString();CoreCommandSchemaV11.markApplied(db,commandId,resultJson,now);db.setTransactionSuccessful();return resultJson;
            }finally{db.endTransaction();}
