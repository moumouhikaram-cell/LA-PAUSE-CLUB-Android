...
public final class CoreStore extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE core_meta (" +
                "key TEXT PRIMARY KEY NOT NULL," +
                "value TEXT NOT NULL)");

        db.execSQL("CREATE TABLE state_snapshots (" +
        ...
        db.execSQL("CREATE INDEX idx_state_snapshots_created " +
                "ON state_snapshots(created_at_ms DESC)");

        db.execSQL("CREATE TABLE resources_shadow (" +
                "id TEXT PRIMARY KEY NOT NULL," +
                "name TEXT NOT NULL," +
    ...
    private void mirrorLegacyState(String json, String source) {
        ...
        try {
            ...
            try (Cursor c = db.rawQuery(
                    "SELECT checksum_sha256 FROM state_snapshots ORDER BY created_at_ms DESC LIMIT 1",
                    null
            )) {
                if (c.moveToFirst() && checksum.equals(c.getString(0))) {
            ...
            if (snapshotNeeded) {
                ...
                db.execSQL(
                        "DELETE FROM state_snapshots WHERE id NOT IN (" +
                                "SELECT id FROM state_snapshots ORDER BY created_at_ms DESC LIMIT " +
                                MAX_SNAPSHOTS + ")"
                );
            }
    ...
    private static void mirrorStations(SQLiteDatabase db, JSONArray stations, long now) {
        ...
        for (int i = 0; i < stations.length(); i++) {
            ...
            String id = station.optString("id", "").trim();
            ...
            v.put("id", id);
            v.put("name", station.optString("name", id));
            v.put("resource_type", resourceType);
            v.put("legacy_station_type", legacyType);
            v.put("enabled", station.optBoolean("enabled", true) ? 1 : 0);
            v.put("sort_order", station.optInt("sort", i + 1));
            v.put("updated_at_ms", now);
            db.insertOrThrow("resources_shadow", null, v);
        }
    }
    ...
    private static void importLegacyEvents(SQLiteDatabase db, JSONArray events, long now) {
        ...
        for (int i = 0; i < events.length(); i++) {
            ...
            String eventId = event.optString("id", "").trim();
            if (eventId.isEmpty()) continue;

            ContentValues v = new ContentValues();
            v.put("event_id", eventId);
            v.put("event_type", event.optString("type", "legacy.unknown"));
            String entityId = event.optString("entityId", "");
            if (!entityId.isEmpty()) v.put("entity_id", entityId);
            Object payload = event.opt("payload");
            v.put("payload_json", payload == null || payload == JSONObject.NULL ? null : String.valueOf(payload));
            v.put("event_at_ms", event.optLong("at", now));
            v.put("legacy_revision", event.optLong("revision", 0L));
            v.put("imported_at_ms", now);

            long inserted = db.insertWithOnConflict(
                    "domain_events", null, v, SQLiteDatabase.CONFLICT_IGNORE
            );
            ...
            if (inserted != -1) {
                ...
                db.insertWithOnConflict(
                        "sync_outbox", null, outbox, SQLiteDatabase.CONFLICT_IGNORE
                );
            }
        }
    }
    ...
    public synchronized String recoverLatestValidStateJson() {
        ...
        try (Cursor c = db.rawQuery(
            ...
            while (c.moveToNext()) {
                ...
                String checksum = c.getString(1);
                try {
                    new JSONObject(json);
                    if (checksum.equals(sha256(json))) return json;
    ...
    public synchronized String getOperatingMode() {
        return getMeta(getReadableDatabase(), "operating_mode", "STANDALONE");
    }

    public synchronized void setOperatingMode(String mode) {
        String normalized = mode == null ? "" : mode.trim().toUpperCase(Locale.ROOT);
        if (!"STANDALONE".equals(normalized) && !"CONNECTED_LOCAL".equals(normalized)) {
            throw new IllegalArgumentException("Unsupported operating mode");
        }
        SQLiteDatabase db = getWritableDatabase();
    ...
    private static void putMeta(SQLiteDatabase db, String key, String value) {
        ...
        db.insertWithOnConflict("core_meta", null, v, SQLiteDatabase.CONFLICT_REPLACE);
    }

    private static String getMeta(SQLiteDatabase db, String key, String fallback) {
        try (Cursor c = db.rawQuery(
                "SELECT value FROM core_meta WHERE key=?",
                new String[]{key}
        )) {
