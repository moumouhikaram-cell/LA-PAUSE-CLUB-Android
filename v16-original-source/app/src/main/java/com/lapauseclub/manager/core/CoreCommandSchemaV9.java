...
final class CoreCommandSchemaV9 {
    ...
    static String statusJson(SQLiteDatabase db){long commands=scalar(db,"SELECT COUNT(*) FROM command_log_v9"),rejected=scalar(db,"SELECT COUNT(*) FROM command_log_v9 WHERE status='REJECTED'"),revisions=scalar(db,"SELECT COUNT(*) FROM entity_revisions_v9"),audits=scalar(db,"SELECT COUNT(*) FROM audit_events_v9");return "{\"commandCount\":"+commands+",\"rejectedCommandCount\":"+rejected+",\"entityRevisionCount\":"+revisions+",\"auditV9Count\":"+audits+"}";}
