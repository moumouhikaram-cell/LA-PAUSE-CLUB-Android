...
const files={
  app:read('app.js'),v15:read('v15.js'),v172:read('v172.js'),runtime:read('master-v2-runtime.js'),
...
requires('universal resources',['p1ResourceType','p1RateFor','startDraftSession','SESSION.REQUEST','SESSION.PAY','SESSION.START','BILLIARD_TABLE','SNOOKER_TABLE','PC_GAMING','SIM_RACING']);
...
const routeCorpus=[files.app,files.v15,files.v172,files.runtime,files.core,files.ops,files.fin,files.commerce,files.device,files.fleet,files.owner,files.intel,files.player,files.experience,files.saas,files.platform,files.client,files.final,files.venue].join('\n');
const missing=[];
for(const target of [...new Set(menuTargets)]){
  if(!routeCorpus.includes(`'${target}'`)&&!routeCorpus.includes(`\"${target}\"`)&&!routeCorpus.includes(target))missing.push(target);
}
...
assert(/android:name="\.PremiumActivity"/.test(fs.readFileSync(path.join(__dirname,'..','app','src','main','AndroidManifest.xml'),'utf8')),'PremiumActivity must be launcher');
