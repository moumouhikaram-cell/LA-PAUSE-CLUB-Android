...
class LocalLoader extends ResourceLoader{
  fetch(url){
    try{
      const u=new URL(url); if(u.hostname!=='lapause.local') return null;
      const rel=decodeURIComponent(u.pathname.replace(/^\/+/,''));
      const p=path.join(A,rel); if(!p.startsWith(A)||!fs.existsSync(p)||fs.statSync(p).isDirectory())return null;
