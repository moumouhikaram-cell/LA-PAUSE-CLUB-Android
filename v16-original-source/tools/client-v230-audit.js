...
has(v230,'if(route!==last)LP.nav.push(route);','history push regression');
not(v230,'route!==LP.canonical(currentView)&&route!==last','old broken push rule returned');
...
has(v230,'const originalAddPayment=window.addPayment','cash payment auto-shift missing');
...
const app=read('app/src/main/assets/app.js');has(app,'ps5Solo:22');has(app,'ps5Duo:28');has(app,'sim:45');
const customer=v230.toUpperCase();['MASTER V2','SYSTEM STATUS','BLOCKED_EXTERNAL','COMMANDENVELOPE','SQLITE DB','CDC OUTPUT'].forEach(x=>not(customer,x,`developer text leaked: ${x}`));
