...
const state={
  ...
  payments:[{id:'p1',sessionId:'s2',amount:22,method:'cash',at:now-86400000}],orders:[{id:'o1',customerId:'c1',status:'paid',total:10,paidAt:now-86400000}],
  clients:[{id:'c1',name:'Karam',firstName:'Karam',lastName:'M',phone:'0600000000',status:'ACTIVE'}],products:[{id:'prod1',name:'Coca',enabled:true,stock:1,alertStock:2}],queue:[],incidents:[],meta:{}
};
...
const ctx={console,Date,Map,Math,JSON,Number,String,Object,Array,Set,queueMicrotask:fn=>fn(),state,uid,
  ...
  saveState:e=>{events.push(e);return true},sessionById,clientById,paymentsForSession,paidForSession,dueForSession,sessionElapsedMinutes,stationLabel,currentShift,addPayment,memberNoV15,
  extendSession:originalExtend,renderView(){},showSheet(){},showModal(){},window:null};
ctx.window=ctx;vm.createContext(ctx);
const files=['enrich-v160-core.js','enrich-v160-billing.js','enrich-v160-session-profiles.js','enrich-v160-revenue.js','enrich-v160-intelligence.js','enrich-v160-player.js','enrich-v160-finance.js','enrich-v160-client.js','enrich-v160-catalog.js'];
for(const f of files)vm.runInContext(fs.readFileSync(path.join(root,f),'utf8'),ctx,{filename:f});
function ok(v,msg){if(!v)throw new Error(msg)}
ok(ctx.LP160,'LP160 runtime missing');
ok(ctx.LP160.modules.size===9,'unexpected module count');
