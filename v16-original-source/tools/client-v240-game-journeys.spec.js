...
async function prepare(page,s){
  await page.evaluate(cfg=>{
    ...
    state.ui=state.ui||{};
    state.ui.opsActivityType=cfg.type;
  ...
  await expect(page.locator(`[data-ux-activity="${s.type}"]`)).toHaveClass(/active/);
  await expect(page.locator(`[data-cs-station="${s.id}"]`)).toBeVisible();
}
...
async function openJourney(page,id){
  ...
  if(await quick.count() && await quick.isVisible()){
    await quick.click();
  }else{
  ...
  const form=page.locator('#opsSessionForm');
  await expect(form).toBeVisible();
  await expect(form).not.toContainText('Image personnalisée');
  await expect(form.locator('input[type="url"]')).toHaveCount(0);
  await expect(form.locator('#opsClient')).toHaveCount(0);
  await expect(form.getByText('Non identifié',{exact:true}).first()).toBeVisible();
  await expect(form.locator('[data-ux-client-open]')).toBeVisible();
  return card;
}
...
test('CONSOLE journey: timed Duo preset, optional game, exact quote/payment',async({page})=>{
  ...
  expect(errors).toEqual([]);console.log('V240_JOURNEY_CONSOLE_OK');
});
...
test('SIM_RACING journey: block package, no player selector, +1 block action',async({page})=>{
  ...
  expect(errors).toEqual([]);console.log('V240_JOURNEY_SIM_RACING_OK');
});
...
test('PC_GAMING journey: budget sale converts amount to minutes and keeps game optional',async({page})=>{
  ...
  expect(errors).toEqual([]);console.log('V240_JOURNEY_PC_GAMING_OK');
});
...
test('BILLIARD_TABLE journey: one game sale, no console fields, +1 game in one action',async({page})=>{
  ...
  expect(errors).toEqual([]);console.log('V240_JOURNEY_BILLIARD_OK');
});
...
test('SNOOKER_TABLE journey: per-player game pricing changes quote before start',async({page})=>{
  ...
  expect(errors).toEqual([]);console.log('V240_JOURNEY_SNOOKER_OK');
});
...
test('TABLE_TENNIS journey: timed group session without game/console noise',async({page})=>{
  ...
  expect(errors).toEqual([]);console.log('V240_JOURNEY_TABLE_TENNIS_OK');
});
...
test('PRIVATE_ROOM journey: fixed package, group capacity, no game fields',async({page})=>{
  ...
  expect(errors).toEqual([]);console.log('V240_JOURNEY_PRIVATE_ROOM_OK');
});
...
test('ARCADE journey: per-game sale, optional title, +1 game in one action',async({page})=>{
  ...
  expect(errors).toEqual([]);console.log('V240_JOURNEY_ARCADE_OK');
});
...
test('CUSTOM journey: operator-entered amount becomes the exact payable amount',async({page})=>{
  ...
  expect(errors).toEqual([]);console.log('V240_JOURNEY_CUSTOM_OK');
});
...
test('all game/resource journeys are explicitly represented',async({page})=>{
  ...
  console.log('V240_METIER_FIRST_GAME_JOURNEYS_OK');
});
