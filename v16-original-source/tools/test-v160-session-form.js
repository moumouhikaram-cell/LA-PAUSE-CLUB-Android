...
const billing={MODEL,typeOf,quote(st,d){const t=typeOf(st);if(t==='CONSOLE'){const rate=d.players===2?28:22;if(d.mode==='open')return {known:false,amount:0,rate};if(d.mode==='budget')return {known:d.budget>0,amount:d.budget,minutes:d.budget/rate*60,rate};return {known:true,amount:rate*d.duration/60,minutes:d.duration,rate};}if(t==='SIM_RACING')return {known:true,amount:45*d.duration/60,minutes:d.duration,rate:45};if(t==='BILLIARD_TABLE')return {known:true,amount:7*d.units,units:d.units,unitPrice:7};if(t==='PC_GAMING')return {known:false,amount:0,minutes:d.duration,rate:0};return {known:false,amount:0};}};
